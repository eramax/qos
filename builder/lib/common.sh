...
repo_root() {
  ...
  #   builder/lib/common.sh -> repo root is ../..
  local lib_dir root
  ...
  [[ -f "$root/builder/lib/common.sh" ]] || die "repo root sanity check failed (missing builder/lib/common.sh): $root"
  [[ -f "$root/builder/build.sh" ]] || die "repo root sanity check failed (missing builder/build.sh): $root"
