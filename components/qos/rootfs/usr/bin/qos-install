...
if [ -x "$MNTBASE/root/usr/bin/cloud-init" ]; then
    chroot "$MNTBASE/root" /usr/bin/cloud-init clean --logs >/dev/null 2>&1 || true
    rm -rf "$MNTBASE/root/var/lib/cloud"/*
    rm -f "$MNTBASE/root/var/log/cloud-init.log" "$MNTBASE/root/var/log/cloud-init-output.log"
fi
