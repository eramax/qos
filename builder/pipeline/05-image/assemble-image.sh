...
layout_src="${IMAGE_LAYOUT:-$root/builder/pipeline/05-image/layout.json}"
slots_src="${IMAGE_SLOTS:-$root/builder/pipeline/05-image/slots.json}"
fstab_src="${IMAGE_FSTAB:-$root/builder/pipeline/05-image/fstab}"
state_template="${STATE_TEMPLATE:-$root/builder/pipeline/05-image/slots.json}"
...
manifest_add "command: builder/pipeline/05-image/assemble-image.sh image=$image_name"
